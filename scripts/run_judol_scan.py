#!/usr/bin/env python3
"""Run Judol (Online Gambling) Domain Scan CLI.
...
judol_findings rows, never removes them.
"""
...
from lib.judol_detect import scan_ct_raw

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(name)s: %(message)s")
logger = logging.getLogger("siaga.run_judol_scan")


def main() -> None:
    parser = argparse.ArgumentParser(description="SIAGA Judol Domain Scan")
    parser.add_argument("--db-path", type=Path, default=BASE_DIR / "data" / "siaga.db")
    parser.add_argument(
        "--allow-llm",
        action="store_true",
        default=False,
        help="Also verify ambiguous-keyword candidates (toto, bola, domino, ...) via LLM judgment. Costs LLM budget.",
    )
    ...
    print("\n" + "=" * 60)
    print("      SIAGA JUDOL DOMAIN SCAN RESULTS")
    print("=" * 60)
    print(f"Domains scanned              : {summary.domains_scanned}")
    print(f"Domains flagged (judol)      : {summary.domains_flagged}")
