...
def create_schema(conn: sqlite3.Connection) -> None:
    """Create/migrate every SIAGA table on an already-open connection.

    Factored out of init_db() so the Vercel serverless in-memory fallback
    (dashboard/api.py's load_in_memory_from_snapshot) can scaffold the full
    real schema on a :memory: connection too -- it previously only created
    3 of the 13 tables from the JSON snapshot, so any query against
    ct_raw/campaigns/judol_findings/porn_findings/message_analyses raised
    "no such table" and crashed the whole request (e.g. the Overview page's
    /api/stats/analytics 500). Does not commit; the caller owns the
